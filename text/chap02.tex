...
\begin{itemize}
    \item Use default implementation by extending the \textit{HttpServletRequest}\footnote{\url{https://docs.oracle.com/javaee/7/api/javax/servlet/http/HttpServletRequest.html}} and its method \textit{doGet()}. The main
...
represent different Lucene \textit{fields} that should be searched. Each of these input fields can contain some value.
...
Each of these fields can include any form of Lucene query, as stated in the Section \ref{opengrok_usage}. Since these queries might
...
\paragraph{FST\protect\footnote{\url{https://en.wikipedia.org/wiki/Finite-state\_transducer}}} \label{FST} –
...
\textbf{Example} – WFST with terms and weights \newline
$CAR = 10, CARS = 15, CART = 5$ can be seen in the Figure \ref{wfst_example}.
...
\begin{itemize}
    \item \textbf{English words} – list of \textbf{370,099} words available at \url{https://github.com/dwyl/english-words}.
    It is distributed under the Unlicense license \citep{unlicense}.
...
\begin{itemize}
    ...
    Results can be seen in the Figure \ref{comp_one}.
    ...
    from the basic alphabet. Results can be seen in the Figure \ref{comp_two}.
    ...
    \item \textbf{Build time} – performs build of the data structure from the scratch. Results can be seen in the Figure
    \ref{comp_build}.
    ...
    \item \textbf{Memory usage} – approximate sizes the data structures use in the computer's main memory.
     ...
     The drastic
    difference is mainly due to WFST implementation which was optimized to use as little memory as possible while
...
How to reproduce these benchmarks and description of the machine on which they were run can be found in Attachment \ref{benchmark_attachment}.
...
\subsection{Promoting Suggestions Using Previous Searches}
...
\begin{itemize}
    \item \textbf{Java Map} implementation which supports concurrent access, e.g. \textit{ConcurrentHashMap}\footnote{\url{https://docs.oracle.com/javase/10/docs/api/java/util/concurrent/ConcurrentHashMap.html}}. This map could be stored
    ...
    \begin{itemize}
        ...
        \textit{Linux kernel} dataset contains approximately 3.5 million terms for \textit{full} field.
    \end{itemize}
...
As mentioned in the Section \ref{possible_implementations}, the best option for simple prefix queries is a WFST data structure.
...
\end{code}
    ...
    \item By processing JSON data:
        \begin{itemize}
            ...
            \item Example data:
\begin{code}
[{"project": "opengrok", "field": "full", "token": "test",
"increment": 1}]
\end{code}
        \end{itemize}
\end{itemize}
...
Since authorization is optional
...
respect the paradigm "best effort"\footnote{\url{https://en.wikipedia.org/wiki/Best-effort\_delivery}}. The suggestions are not
...
\begin{itemize}
    ...
    \item \textbf{all} – if specified to \texttt{true}, then all the data are retrieved.
\end{itemize}
