...
At a first glance, it might seem as a good idea to use \textit{inverse document frequency} (IDF) as a metric. The formula for computing this metric can be seen at \ref{IDF_eq}.
\begin{equation}
\label{IDF_eq}
IDF(t) = \log{\frac{D}{N}}
\end{equation}
...
Resultant equation can be seen at \ref{norm_doc_freq}.
\begin{equation}
\label{norm_doc_freq}
...
\begin{itemize}
    ...
    Results can be seen on the graph \ref{comp_one}.
...
As specified in \ref{opengrok_overview}, OpenGrok allows the administrators to specify to which projects the user has
...
\begin{itemize}
    ...
    The value must be in the Unix cron format, decision is described in \ref{cron}.  Default value: \textit{0 0 * * *} (every day at midnight).
...
In \ref{promoting_on_previous_searches} it was discussed that most popular completion will be used. Many users or
